...
def load_demo_images(images_dir):
    ...
    for image in os.listdir(images_dir):
        ...
        img = img.resize((img_h, img_w), Image.Resampling.LANCZOS)
...
def main():
    ...
    os.makedirs(os.path.dirname(pred_file_name), exist_ok=True)
    images_dir = sys.argv[2] if len(sys.argv) > 2 else None

    # load images
    demo_imgs = load_demo_images(images_dir)
